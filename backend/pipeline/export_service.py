...
class ExportFormat(str, Enum):
    """The serialization used for an exported pipeline snapshot."""
...
class PipelineExport:
    """A single exported pipeline section, serialized to text."""
...
class PipelineExportService:
    """Exports pipeline definitions, run history, and schema snapshots for backup/integration."""
    ...
    def _gather_definitions(self, names: Optional[list] = None) -> list:
    ...
    def _gather_runs(self, *, workflow_name: Optional[str] = None) -> list:
        return [run.to_dict() for run in self._executor.list_runs(workflow_name=workflow_name)]

    def _gather_schemas(self) -> list:
        return [schema.to_dict() for schema in self._schemas.list_schemas()]

    def _build_export(self, section: str, data: Any, fmt: ExportFormat) -> PipelineExport:
    ...
    def export_pipeline(self, *, fmt: ExportFormat = ExportFormat.JSON, names: Optional[list] = None) -> PipelineExport:
        return self._build_export("definitions", self._gather_definitions(names), fmt)

    def export_runs(
        self, *, fmt: ExportFormat = ExportFormat.JSON, workflow_name: Optional[str] = None
    ) -> PipelineExport:
        return self._build_export("runs", self._gather_runs(workflow_name=workflow_name), fmt)

    def export_schemas(self, *, fmt: ExportFormat = ExportFormat.JSON) -> PipelineExport:
        return self._build_export("schemas", self._gather_schemas(), fmt)
    ...
    def export_all(self, *, fmt: ExportFormat = ExportFormat.JSON) -> ExportManifest:
        ...
        return ExportManifest(
            export_id=export_id,
            sections=("definitions", "runs", "schemas", "analytics"),
            format=fmt,
...
_pipeline_export_service: Optional[PipelineExportService] = None


def get_pipeline_export_service() -> PipelineExportService:
    global _pipeline_export_service
    if _pipeline_export_service is None:
        _pipeline_export_service = PipelineExportService(
            registry=get_pipeline_registry(),
            executor=get_pipeline_execution_engine(),
            schemas=get_schema_registry(),
            analytics=get_pipeline_analytics_service(),
        )
    return _pipeline_export_service
