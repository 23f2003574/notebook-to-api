...
def test_generate_python_sdk_constructor_accepts_a_configurable_timeout(tmp_path):
    ...
    # Every requests.* call this client makes must actually use it: the 10
...
def test_generate_typescript_sdk_constructor_accepts_a_configurable_timeout(
    ...
    # Every fetch() call this client makes must actually use it: the 10
...
def test_generate_python_sdk_retry_task_sends_correct_request(tmp_path, monkeypatch):
    """POST /tasks/{task_id}/retry already exists server-side (retry_task
    in api_generator.py) and actually re-executes a failed task's own
    underlying function, unlike redeliver_task_webhook above -- but
    nothing let a caller trigger that through the generated client
    itself.
    """
    ...
    class FakeResponse:
        ...
        def json(self):
            return {
                "task_id": "def456", "status": "processing",
                "retried_from": "abc123",
            }
    ...
    client = namespace["NotebookAPIClient"]("http://localhost:8000")
    result = client.retry_task("abc123")

    assert result == {
        "task_id": "def456", "status": "processing", "retried_from": "abc123",
    }
    assert calls == [
        {
            "url": "http://localhost:8000/tasks/abc123/retry",
            "headers": {"X-API-Key": "notebook-to-api-dev-key"},
        }
    ]


def test_generate_python_sdk_cleanup_tasks_sends_correct_request(tmp_path, monkeypatch):
    """POST /tasks/cleanup already exists server-side (cleanup_tasks in
    api_generator.py) but had no client method of its own, the same gap
    delete_completed_tasks/delete_failed_tasks above already closed for
    the two single-status deletes it combines.
    """
    ...
    class FakeResponse:
        ...
        def json(self):
            return {"completed_deleted": 4, "failed_deleted": 2, "remaining_tasks": 1}
    ...
    assert calls == [
        {
            "url": "http://localhost:8000/tasks/cleanup",
            "headers": {"X-API-Key": "notebook-to-api-dev-key"},
        }
    ]


def test_generate_python_sdk_reset_tasks_sends_correct_request(tmp_path, monkeypatch):
    """POST /tasks/reset already exists server-side (reset_tasks in
    api_generator.py) but had no client method of its own.
    """
    ...
    client = namespace["NotebookAPIClient"]("http://localhost:8000")
    result = client.reset_tasks()

    assert result == {"deleted_tasks": 7}
    assert calls == [
        {
            "url": "http://localhost:8000/tasks/reset",
            "headers": {"X-API-Key": "notebook-to-api-dev-key"},
        }
    ]


def test_generate_python_sdk_retry_task_name_takes_priority_over_a_colliding_path(
    tmp_path,
):
    """Same collision hazard as redeliver_task_webhook's own test above,
    for the newly hardcoded retry_task method.
    """

    schema_path = _write_schema(
        tmp_path,
        {"/retry_task": {"post": {"operationId": "retry_task"}}},
    )
    output_path = tmp_path / "client.py"
...
def test_generate_typescript_sdk_retry_task_sends_correct_request(tmp_path):
    """Mirrors test_generate_python_sdk_retry_task_sends_correct_request
    for the TypeScript client.
    """
    ...
    runner_path.write_text(
        ...
        const result = await client.retryTask("abc123");
    ...
    output = json.loads(proc.stdout.strip().splitlines()[-1])
    assert output["calls"] == [
        {"url": "http://localhost:8000/tasks/abc123/retry", "method": "POST"}
    ]
    assert output["result"] == {
        "task_id": "def456", "status": "processing", "retried_from": "abc123",
    }
...
def test_generate_typescript_sdk_cleanup_and_reset_tasks_send_correct_requests(
    tmp_path,
):
    """Mirrors test_generate_python_sdk_cleanup_tasks_sends_correct_request
    and test_generate_python_sdk_reset_tasks_sends_correct_request for the
    TypeScript client, in one run.
    """
    ...
    runner_path.write_text(
        ...
        globalThis.fetch = async (url, opts) => {{
          ...
          return {{ ok: true, json: async () => ({{ deleted_tasks: 7 }}) }};
        }};

        const {{ NotebookAPIClient }} = await import({json.dumps(str(client_path))});
        const client = new NotebookAPIClient("http://localhost:8000");

        const cleanupResult = await client.cleanupTasks();
        const resetResult = await client.resetTasks();

        console.log(JSON.stringify({{ calls, cleanupResult, resetResult }}));
        """,
        encoding="utf-8",
    )
    ...
    output = json.loads(proc.stdout.strip().splitlines()[-1])
    assert output["calls"] == [
        {"url": "http://localhost:8000/tasks/cleanup", "method": "POST"},
        {"url": "http://localhost:8000/tasks/reset", "method": "POST"},
    ]
    assert output["cleanupResult"] == {
        "completed_deleted": 4, "failed_deleted": 2, "remaining_tasks": 1,
    }
    assert output["resetResult"] == {"deleted_tasks": 7}
